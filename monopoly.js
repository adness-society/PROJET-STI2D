...
function Game() {
	...
	var finalizeAuction = function() {
		...
		if (highestbid > 0) {
			...
			addAlert(p.name + " Acheté " + sq.name + " pour $" + highestbid + ".");
		}
	...
	this.auction = function() {
		...
		popup("<div style='font-weight: bold; font-size: 16px; margin-bottom: 10px;'>Vente aux enchères <span id='propertyname'></span></div><div>L'offre la plus élevée = $<span id='highestbid'></span> (<span id='highestbidder'></span>)</div><div><span id='currentbidder'></span>, c'est à vous d'enchérir.</div<div><input id='bid' title='Saisir un montant pour l'enchère " + s.name + ".' style='width: 291px;' /></div><div><input type='button' value='Offre' onclick='game.auctionBid();' title='Placez votre offre.' /><input type='button' value='Passer' title='Ne pas faire d'offres cette fois-ci.' onclick='game.auctionPass();' /><input type='button' value='Quitter' title='Arrêter de faire des offres sur " + s.name + " au total.' onclick='if (confirm(\"Êtes-vous sûr de vouloir cesser d'enchérir sur ce bien ?\")) game.auctionExit();' /></div>", "blank");
	...
	var resetTrade = function(initiator, recipient, allowRecipientToBeChanged) {
		...
		if (initiatorSideTable.lastChild) {
			...
			initiatorProperty.textContent = initiator.name + " n'a pas de propriétés à échanger.";
		}

		if (recipientSideTable.lastChild) {
			recipientProperty.appendChild(recipientSideTable);
		} else {
			recipientProperty.textContent = recipient.name + " n'a pas de propriétés à échanger.";
		}
		...
		if (allowRecipientToBeChanged && pcount > 2) {
			...
			nameSelect.title = "Selectionne un joueur pour échanger avec.";
		} else {
	...
	this.acceptTrade = function(tradeObj) {
		...
		if (isAPropertySelected === 0) {
			popup("<p>Une ou plusieurs propriétés doivent être sélectionnées afin d'effectuer des échanges.</p>");

			return false;
		}

		if (showAlerts && !confirm(initiator.name + ", êtes-vous sûr de vouloir faire cet échange avec " + recipient.name + "?")) {
			return false;
		}
		...
		for (var i = 0; i < 40; i++) {
			...
			if (tradeObj.getProperty(i) === 1) {
				...
				addAlert(recipient.name + " reçoit " + square[i].name + " de " + initiator.name + ".");
			} else if (tradeObj.getProperty(i) === -1) {
				square[i].owner = initiator.index;
				addAlert(initiator.name + " reçoit " + square[i].name + " de " + recipient.name + ".");
			}
		...
		if (tradeObj.getCommunityChestJailCard() === 1) {
			...
			addAlert(recipient.name + ' a reçu une carte "sortir de prison gratuitement" de la part de ' + initiator.name + ".");
		} else if (tradeObj.getCommunityChestJailCard() === -1) {
			initiator.communityChestJailCard = true;
			recipient.communityChestJailCard = false;
			addAlert(initiator.name + ' a reçu une carte "sortir de prison gratuitement" de la part de ' + recipient.name + ".");
		}
		...
		if (money > 0) {
			...
			addAlert(recipient.name + " reçoit $" + money + " de " + initiator.name + ".");
		} else if (money < 0) {
			money = -money;
			...
			addAlert(initiator.name + " reçoit $" + money + " de " + recipient.name + ".");
		}
	...
	this.proposeTrade = function() {
		...
		if (initiator.human && !confirm(initiator.name + ", êtes-vous sûr de vouloir faire cet échange avec " + recipient.name + "?")) {
			return false;
		}
...
function updateDice() {
	...
	if (document.images) {
		...
		element0.title = "Dé (" + die0 + " Point(s))";
		element1.title = "Dé (" + die1 + " Point(s))";
...
function updateOwned() {
	...
	if (HTML === "") {
		HTML = p.name + ", vous n'avez pas de propriétés.";
		$("#option").hide();
	} else {
		HTML += "</table>";
	}
...
function gotojail() {
	...
	document.getElementById("landed").innerHTML = "Vous êtes en prison.";

	p.jail = true;
	doublecount = 0;

	document.getElementById("nextbutton").value = "Fin du tour";
	document.getElementById("nextbutton").title = "Fin du tour et passage au joueur suivant.";
...
function streetrepairs(houseprice, hotelprice) {
	...
	if (cost > 0) {
		...
		if (houseprice === 40) {
			addAlert(p.name + " a perdu $" + cost + " pour Community Chest.");
		} else {
			addAlert(p.name + " a perdu $" + cost + " pour Chance.");
		}
	}
...
function showStats() {
	...
	for (var x = 1; x <= pcount; x++) {
		...
		if (!write) {
			HTML += p.name + " n'a pas de propriétés.";
		} else {
			HTML += "</table>";
		}
	...
	HTML += "</tr></table><div id='Tritre'></div>";
...
function buy() {
	...
	if (p.money >= cost) {
		...
		property.owner = turn;
		updateMoney();
		addAlert(p.name + " A acheté " + property.name + " pour " + property.pricetext + ".");
...
function mortgage(index) {
	...
	document.getElementById("mortgagebutton").value = "Annuler l'hypothèque pour $" + unmortgagePrice;
	document.getElementById("mortgagebutton").title = "Annuler l'hypothèque " + sq.name + " pour $" + unmortgagePrice + ".";
...
function unmortgage(index) {
	...
	sq.mortgage = false;
	document.getElementById("mortgagebutton").value = "Hypothèque pour $" + mortgagePrice;
	document.getElementById("mortgagebutton").title = "Hypothèque " + sq.name + " pour $" + mortgagePrice + ".";
...
function land(increasedRent) {
	...
	$("#landed").show();
	document.getElementById("landed").innerHTML = "Vous êtes tombé sur " + s.name + ".";
	s.landcount++;
	addAlert(p.name + " tombé sur " + s.name + ".");
	...
	if (s.price !== 0 && s.owner === 0) {
		...
		if (!p.human) {
			...
			document.getElementById("landed").innerHTML = "<div>Vous êtes tombé sur <a href='javascript:void(0);' onmouseover='showdeed(" + p.position + ");' onmouseout='hidedeed();' class='statscellcolor'>" + s.name + "</a>.<input type='button' onclick='buy();' value='Acheter ($" + s.price + ")' title='Acheter " + s.name + " pour " + s.pricetext + ".'/></div>";
		}
	...
	if (s.owner !== 0 && s.owner != turn && !s.mortgage) {
		...
		addAlert(p.name + " paye $" + rent + " pour " + player[s.owner].name + ".");
		p.pay(rent, s.owner);
		player[s.owner].money += rent;

		document.getElementById("landed").innerHTML = "Vous êtes tombé sur " + s.name + ". " + player[s.owner].name + " collecté $";
	} else if (s.owner > 0 && s.owner != turn && s.mortgage) {
		document.getElementById("landed").innerHTML = "Vous êtes tombé sur " + s.name + ". La propriété est hypothéquée ; aucun loyer n'a été perçu.";
	}
...
function roll() {
	...
	if (die1 == die2) {
		addAlert(p.name + " tombé sur " + (die1 + die2) + " - double.");
	} else {
		addAlert(p.name + " tombé sur " + (die1 + die2) + ".");
	}
	...
	if (die1 == die2 && !p.jail) {
		...
		if (doublecount < 3) {
			document.getElementById("nextbutton").value = "Relance";
			document.getElementById("nextbutton").title = "Vous avez fait un double. Relance.";
		...
		// If player rolls doubles three times in a row, send him to jail
			...
			if (p.human) {
				popup("Vous avez obtenu des doubles trois fois de suite. Aller en prison.", gotojail);
			} else {
				gotojail();
			}
		...
		document.getElementById("nextbutton").value = "Passer aux enchères";
		document.getElementById("nextbutton").title = "Organiser des enchères.";
		doublecount = 0;
	}
...
function play() {
	...
	addAlert("C'est a " + p.name + " de jouer.");
	...
	document.getElementById("nextbutton").value = "Lancer les dés";
	document.getElementById("nextbutton").title = "Lancez les dés et déplacez votre pion en conséquence.";
	...
	if (p.jail) {
		$("#landed").show();
		document.getElementById("landed").innerHTML = "Vous êtes en prison.<input type='button' title='Payer $50 pour sortir immédiatement de prison.' value='Payer une amende de $50' onclick='payfifty();' />";
